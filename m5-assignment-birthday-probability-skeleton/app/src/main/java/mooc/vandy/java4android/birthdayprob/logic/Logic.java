        ...
        implements LogicInterface
{
    ...
    {
        int countOfEqualBirthDays = 0;
        for (int i = 0; i < count; i++)
            countOfEqualBirthDays += newTest(i, size);
        return (double)(countOfEqualBirthDays) * 100 / count;
    }
    
    /**
     * This method performs one test.
     */
    public int newTest(int i, int size)
    {
        Set<Integer> birthDaysSet = new HashSet<Integer>();
        Random rnd = new Random(i + 1);
        for (int j = 0; j < size; j++)
        {
            int newBirthDay = rnd.nextInt(365);
            if (birthDaysSet.contains(newBirthDay))
                return 1;
            else
                birthDaysSet.add(newBirthDay);
        }
        return 0;
    }
}
